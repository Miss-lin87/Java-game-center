...
public class general_functions {
    static String regex_spell_correct(String[] text_input, String text_player){
    ...
    static int computer_random(int max){
        int random_pick = (int)(Math.random()*max);
        return random_pick;}

    static int random_number_range(int start, int stop){
        int value = (int)(Math.random()*stop);
        while (value < start){return random_number_range(start, stop);}
        return value;}

    static HashSet<Integer> create_hash_int_range(int start, int stop){
